  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      if (!result.success) {
        setMessage({
          type: "error",
          text: result.message || "Quantity is not available",
        });
        return;
      }
...
//         const description =
//           data.custom_attributes?.find((a: any) => a.attribute_code === "description")?.value || "";

//         const image = data.media_gallery_entries?.[0]?.file
//           ? `${BASE_URL}/media/catalog/product${data.media_gallery_entries[0].file}`
//           : "/no-image.png";

//         const stock = data.extension_attributes?.quantity ?? data.extension_attributes?.stock_item?.qty ?? 0;
...
//         if (editMode && cartItem) setQty(cartItem.qty);
//       } catch (err) {
//         console.error(err);
//         setMessage({ type: "error", text: "Failed to load product" });
//       }
...
//     try {
//       const result = await addToCart(product, qty);
//       if (!result.success) throw new Error(result.message || "Quantity not available");
//       await refreshCart();
//       openDrawer();
//       setMessage({ type: "success", text: "Product added successfully" });
//     } catch {
...
//       <div className="flex flex-col md:flex-row gap-6">
//         <img src={product.image} alt={product.title} className="w-full md:w-1/2 h-80 object-contain" />

//         <div className="md:w-1/2">
//           <h1 className="text-2xl font-bold">{product.title}</h1>
//           <p className="text-xl font-semibold mt-2">₹{product.price}</p>
//           <p className={`mt-2 ${product.stock > 0 ? "text-green-600" : "text-red-600"}`}>Stock Available: {product.stock}</p>

//           {product.description && <div className="mt-4 text-sm" dangerouslySetInnerHTML={{ __html: product.description }} />}

//           <div className="flex items-center gap-4 mt-6">
//             <button className="px-3 py-1 border" onClick={() => { setQty((q) => Math.max(1, q - 1)); if (editMode) setShowUpdate(true); }}>-</button>
//             <span className="font-semibold">{qty}</span>
//             <button className="px-3 py-1 border" onClick={() => { setQty((q) => q + 1); if (editMode) setShowUpdate(true); }}>+</button>
//           </div>

//           {editMode ? (
//             <button onClick={handleUpdate} disabled={!showUpdate || loading} className={`mt-6 px-6 py-3 text-white rounded ${showUpdate ? "bg-black" : "bg-gray-400"}`}>
//               {loading ? "Updating..." : "Update"}
//             </button>
//           ) : (
//             <button onClick={handleAddToCart} disabled={loading} className="mt-6 px-6 py-3 bg-black text-white rounded">
//               {loading ? "Adding..." : "Add to Cart"}
//             </button>
//           )}
//         </div>
...
//           {product.description && (
//             <div className="mt-4 text-sm" dangerouslySetInnerHTML={{ __html: product.description }} />
//           )}
...
//               onClick={handleUpdate}
//               disabled={!showUpdate || loading}
//               className={`mt-6 px-6 py-3 text-white rounded ${showUpdate ? "bg-black" : "bg-gray-400"}`}
//             >
//               {loading ? "Updating..." : "Update"}
//             </button>
//           ) : (
